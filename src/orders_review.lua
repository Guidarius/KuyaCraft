...
function R:draw()
 local g=love.graphics;local w,h=g.getDimensions();g.push('all');g.clear(.105,.135,.13)
 ...
 if self.page==1 then
  ...
  for row,zoom in ipairs({1,1.5,2,1.5}) do
 ...
 elseif self.page==2 then
  for row,id in ipairs(buildings) do for col=1,4 do
   self:sample(id,col==4 and 'idle' or 'construction','S',(col-.5)*w/4,160+row*(h-200)/4,1.3,1,col==4 and 1 or col)
   self:sample('associate','idle','SW',(col-.5)*w/4+100,160+row*(h-200)/4,1.3,1)
  end end
 elseif self.page==4 then
  ...
  for group,army in ipairs(armies) do for row=1,6 do for col=1,12 do
   ...
   self:sample(id,'move',Frames.directions[(row+col-2)%8+1],(group-1)*w/3+45+col*(w/3-80)/12,155+row*68,1.5,1)
  end end end
  for i,pair in ipairs({{'keep','orbital_command'},{'depot','med_bay'},{'barracks','mc_barracks'},{'sanctum','armory'}}) do
   for j,id in ipairs(pair) do self:sample(id,'idle','S',(i-1)*w/4+w/8+(j-1.5)*160,h-65,1.25,1) end
  end
 else
 ...
 g.draw(self.canvas);g.setShader();g.setColor(.96,.93,.83)
 ...
 if self.labels then
  if self.page==1 then
   for row,text in ipairs({'Native 1x / same team','1080p normalized gameplay scale ~1.5x / same team','2x inspection / opposing teams','Movement / same team / light terrain'}) do g.print(text,25,115+(row-1)*(h-135)/4) end
   local roster={};for _,id in ipairs(units) do roster[#roster+1]=id end;for _,id in ipairs(compare) do roster[#roster+1]=id end

...
function A:add(key,text,x,y,app)
 ...
 local item={text=text,x=x,y=y,time=self.clock,priority=text:find('Hero') and 5 or (text:find('Headquarters') or text:find('both control points')) and 4 or text:find('upgrade') and 3 or 1};table.insert(self.items,1,item);table.sort(self.items,function(a,b) if a.priority~=b.priority then return a.priority>b.priority end return a.time>b.time end);if #self.items>4 then table.remove(self.items) end

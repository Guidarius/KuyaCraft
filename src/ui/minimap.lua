...
function M.cache(app)
 ...
 if cache.tick~=app.view.tick or cache.player~=app.player then signature=require('src.sim.codec').encode({app.view.player.visible,app.view.player.explored});cache.tick=app.view.tick;cache.player=app.player end
...
function M.draw(app,panel)
 ...
 for _,e in ipairs(app.observation.markers or {}) do
  ...
  local hero=require('src.content').units[e.kind] and require('src.content').units[e.kind].hero
  ...
  if require('src.ui.selection').has(app.selected,e.id) then g.setColor(.7,1,.7);g.circle('line',x,y,4) end
 end
 ...
 local vp=require('src.ui.camera').rect(app);local x1,y1=app:position(vp.x,vp.y);local x2,y2=app:position(vp.x+vp.w,vp.y+vp.h)

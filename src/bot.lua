...
function B.commands(view,C)
 ...
 local used={};local function build(kind,cx,cy)
  ...
  for r=3,10 do for y=math.max(0,cy-r),math.min(view.map.height-d.size,cy+r) do for x=math.max(0,cx-r),math.min(view.map.width-d.size,cx+r) do
   if math.max(math.abs(x-cx),math.abs(y-cy))==r then
    local room=true
    -- Preserve a two-cell working apron around existing footprints/mines.
    for _,e in ipairs(view.entities) do if e.alive and e.category~='unit' and e.resource~='lumber' then local ex,ey=math.floor(e.x/256),math.floor(e.y/256);if x<ex+e.size+2 and x+d.size>ex-2 and y<ey+e.size+2 and y+d.size>ey-2 then room=false;break end end end
    if room and Sim.placement(view,C,kind,x,y) then spend(d.cost);used[builder.id]=true;add('build',builder,{building=kind,x=x,y=y});return true end
   end
  end end end
  return false
 end
 ...
 local mines={};for _,n in ipairs(nodes) do if n.resource=='gold' then
  local close=(n.x-hq.x)^2+(n.y-hq.y)^2<(16*256)^2
  for _,b in ipairs(outposts) do if b.remaining==0 and (n.x-b.x)^2+(n.y-b.y)^2<(16*256)^2 then close=true end end
  if close then mines[#mines+1]=n end
 end end
 table.sort(mines,function(a,b) local da=(a.x-hq.x)^2+(a.y-hq.y)^2;local db=(b.x-hq.x)^2+(b.y-hq.y)^2;return da<db or da==db and a.id<b.id end)
 local assigned=0
 for _,e in ipairs(workers) do if not used[e.id] and e.order.kind~='build' then
  ...
  if not target then local best;for _,n in ipairs(nodes) do if n.resource=='lumber' then local dist=(n.x-e.x)^2+(n.y-e.y)^2;if not best or dist<best then best=dist;target=n end end end end
 ...
 if #army>=3 and view.tick<6000 then
  for _,e in ipairs(view.entities) do if e.alive and e.home and (e.campTier=='easy' or #army>=6) then local dist=(e.x-hq.x)^2+(e.y-hq.y)^2;if not best or dist<best then target=e;best=dist end end end
 end
 local tx,ty;local threat,threatDistance
 for _,e in ipairs(view.entities) do
  if e.alive and e.owner>0 and e.owner~=owner then local distance=(e.x-hq.x)^2+(e.y-hq.y)^2
   if distance<=(22*256)^2 and (not threatDistance or distance<threatDistance) then threat=e;threatDistance=distance end
  end
 end

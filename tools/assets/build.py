...
PROPS = ['drop_pod']
ORDERS_UNITS = ['worker','worker_loaded','footman','crossbow','gryphon','reliquary']
ORDERS_BUILDINGS = ['keep','depot','barracks','sanctum']
...
def main(argv=None):
    ...
    ap.add_argument('--unit',action='append'); ap.add_argument('--roster',choices=['bastion','woodland','megacorp_aircraft','megacorp_infantry','megacorp_buildings','megacorp_props','orders_units','orders_buildings'],default='bastion')
    ...
    units = args.unit or (ORDERS_UNITS if args.roster == 'orders_units' else ORDERS_BUILDINGS if args.roster == 'orders_buildings' else AIRCRAFT if args.roster == 'megacorp_aircraft' else INFANTRY if args.roster == 'megacorp_infantry' else BUILDINGS if args.roster == 'megacorp_buildings' else PROPS if args.roster == 'megacorp_props' else ROSTER)
    if any(u not in ROSTER+AIRCRAFT+INFANTRY+BUILDINGS+PROPS+ORDERS_UNITS+ORDERS_BUILDINGS for u in units): raise ValueError('Unknown asset; choose '+', '.join(ROSTER+AIRCRAFT+INFANTRY+BUILDINGS+PROPS+ORDERS_UNITS+ORDERS_BUILDINGS))
    ...
    if any(u in ROSTER+INFANTRY+['footman','gryphon'] for u in units) and not source.is_file() and args.mode != 'legacy': raise ValueError('Saved Blender source missing: '+str(source))
    ...
    for unit in units:
        ...
        key, deps = build_key(root,None if unit in AIRCRAFT+BUILDINGS+PROPS+ORDERS_BUILDINGS+['reliquary'] else source,unit,version)

...
function R.register(test)
    ...
    test('crowd','shipping shared routes finish a mixed 50 versus 50 counterflow',function()
        local w=R.world();S.wall(w,31,0,29);S.wall(w,31,32,63)
        local units,commands,goals={},{},{}
        for p=1,2 do for i=1,50 do
            local e=S.unit(w,({'worker','footman','crossbow','enforcer'})[(i-1)%4+1],p,
        ...
        for tick=1,6000 do
            ...
            for _,e in ipairs(units) do
                assert(not e.lastOrderFailure,'shared route abandoned a destination')
                local g=goals[e.id]
                if e.order.kind~='stop' or F.distance2(e.x,e.y,g.x,g.y)>256^2 then done=false end
            end

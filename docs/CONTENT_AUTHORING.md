...
5. Give the unit a producer: list it in a building's `produces`, mark it `pod = true` with a `requires` building for the Megacorp, or list it in a faction `roster` for the legacy war hall. A faction is declared by the schema below; a hero-and-roster faction is the legacy shape and still validates.
6. (Fixture heroes only.) Give each upgrade milestone exactly two choices with meaningful tradeoffs.
...
Current cancellation default: `rules.cancelRefundPercent` (75% in the shipping content), rounded down per resource; 100% refund for an unstarted queue item. Hero progression and revival numbers apply to the fixture only.
...
[ABILITIES.md](ABILITIES.md). `src/content.lua` is authoritative; [FACTIONS.md](FACTIONS.md)
records the current values and [BALANCE_AND_PACING.md](BALANCE_AND_PACING.md) the scale,
map and pacing targets.
